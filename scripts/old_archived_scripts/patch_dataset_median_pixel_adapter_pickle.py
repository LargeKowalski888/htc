...
feather_path = tables_dir / "Cat_HTC_Adapter@median_spectra@semantic#primary.feather"
pickle_path = tables_dir / "Cat_HTC_Adapter@median_spectra@semantic#primary.pkl"

import pandas as pd

df = pd.read_feather(feather_path).reset_index(drop=True)
df.to_pickle(pickle_path)
print(f"Wrote pickle cache: {pickle_path}")
...
class DatasetMedianPixelAdapter(HTCDataset):
    def __init__(self, *args, **kwargs):
        ...
        adapter_root = DataPath.from_image_name(
            f"{requested_image_names[0]}@{annotation_name}"
        ).dataset_settings["dataset_name"]

        # Use environment variable directly to avoid HTC median_table feather reader crash.
        import os
        root = os.environ["PATH_Tivita_Cat_HTC_Adapter"]

        table_path = (
            f"{root}/intermediates/tables/"
            f"{dataset_name}@median_spectra@{annotation_name}.pkl"
        )
        ...
        self.image_labels = torch.from_numpy(np.stack(df["image_labels"])) if image_labels_enabled and "image_labels" in df.columns else None
...
backup = dataset_path.with_suffix(dataset_path.suffix + ".bak_pickle_reader")
