...
class DataPathAdapter(DataPathTivita):
    ...
    def _load_image_name_cache(cls):
        ...
        if not csv_path.exists():
            feather_path = tables_dir / "Cat_HTC_Adapter@meta.feather"
    ...
    def from_image_name(cls, image_name: str):
        ...
        if base_image_name not in cache:
            raise AssertionError(
                f"Could not find {base_image_name} in adapter meta cache "
                f"(n={len(cache)})"
            )
...
backup = path.with_suffix(path.suffix + ".bak_fast_image_name")
